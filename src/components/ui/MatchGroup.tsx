    ...
    <button 
      ...
      onKeyDown={(e) => {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault();
          onOpenAnalysis?.(picks);
        }
      }}
      aria-label={`Ver análisis de ${normalizeTeamName(homeRaw)} contra ${normalizeTeamName(awayRaw)}`}
      className={cn(
        "group relative flex items-center justify-between px-4 py-4 rounded-xl w-full text-left",
    ...
    >
        ...
        {/* Info Section */}
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm font-bold text-text-primary tracking-tight">
              {normalizeTeamName(homeRaw)} <span className="text-text-muted mx-0.5">vs</span> {normalizeTeamName(awayRaw)}
            </span>
          </div>
          <div className="flex items-center gap-1.5 text-xs font-medium text-text-muted uppercase">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2.5 px-3 py-1.5 rounded-full bg-bg-subtle border border-border-base group-hover:border-border-accent/40 transition-all duration-300">
          ...
          <span className="text-[11px] font-bold text-text-muted uppercase tracking-[0.2em] leading-none">
      ...
      >
        <div className="flex items-center gap-6 flex-1 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            <span className="text-xs font-black text-text-muted uppercase tracking-[0.3em] mb-1.5">
              ...
              <div className="flex items-center gap-3">
                ...
                {pick.status !== 'pending' ? (
                  <div className={cn(
                    "w-[64px] py-0.5 rounded-sm text-[10px] font-black uppercase tracking-widest shadow-sm flex items-center justify-center",
                    pick.status === 'won' ? "bg-win text-black" :
                    pick.status === 'lost' ? "bg-loss text-white" :
                    "bg-void text-white"
                  )}>
                  ...
                  <div className="flex items-center gap-1 px-1 py-0.5 rounded-sm bg-white/[0.03] border border-white/10">
                    ...
                    <span className="text-[10px] font-black text-white/20 uppercase tracking-widest italic">Pendiente</span>
                  </div>
                )}

                {isTopPick && pick.status === 'pending' && (
                  <div className="px-1.5 py-0.5 rounded-sm bg-accent/10 border border-accent/20">
                    <span className="text-[11px] font-black text-accent uppercase tracking-widest">TOP</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="flex items-center gap-8">
          {calculatedBet && Number(calculatedBet) > 0 && (
            <div className="hidden md:flex flex-col items-end gap-0.5">
              <span className="text-[10px] font-black text-accent uppercase tracking-widest">APUESTA SUGERIDA</span>
